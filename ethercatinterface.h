 ...
 *   NOTE: 0x4D00:01 and 0x4D01:00 are NOT in this PDO — use async SDO requests instead
 *
...
/*
 * get_digital_output_offsets — retrieves cached 0x60FE:01/02 domain byte offsets.
 * Must be called after setup_domain_sizing() succeeds.
 * off_mask → 0x60FE:01 (Digital Output Mask)
 * off_val  → 0x60FE:02 (Digital Output Value)
 */
 ...
 * each cycle — no blocking, no deadlock, safe to trigger from any goroutine.
 *
 ...
 *   1. trigger_mt_request_step(0, 0x0031)      — set 0x4D01:00 = function code
 *   2. poll get_mt_request_state(0) == 1        — wait for ACK
 *   3. trigger_mt_request_step(1, 0x00000200)   — pulse 0x4D00:01 trigger
 *   4. poll get_mt_request_state(1) == 1
 *   5. trigger_mt_request_step(1, 0x00000000)   — clear trigger
