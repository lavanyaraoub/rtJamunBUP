...
/* RxPDO 0x1600 — 6 entries matching EEPROM default (verified after power cycle).
 ...
 * The Panasonic A6 rejects this → slave stays in PreOp forever.
...
/* TxPDO 0x1A00 — 8 entries matching EEPROM default (verified after power cycle) */
...
/* ============================================================
 * Hardcoded domain byte offsets — kept for reference ONLY.
...
/* ============================================================
 * setup_domain_sizing — registers all PDO entries with the domain.
...
int setup_domain_sizing(ec_domain_t *domain,
    ...
    g_off_cw = _cw; g_off_op = _op; g_off_tp = _tp; g_off_tv = _tv;
    g_off_ec = _ec; g_off_sw = _sw; g_off_pa = _pa; g_off_di = _di;
    g_off_fe1 = _fe1; g_off_fe2 = _fe2;
    g_off_valid = 1;
...
/* ============================================================
 * get_digital_output_offsets — returns cached 0x60FE:01/02 byte offsets.
...
int get_digital_output_offsets(unsigned int *off_mask, unsigned int *off_val) {
    if (!g_off_valid) return -EINVAL;
    *off_mask = g_off_fe1;
    *off_val  = g_off_fe2;
    return 0;
}
/* get_multiturn_reset_offsets is removed — 0x4D01/0x4D00 are not in PDO domain.
 ...
 *   - State transitions: UNUSED → QUEUED → BUSY → SUCCESS/ERROR
 *   - No blocking, no deadlock, safe while PDO cyclic is running
 *
 * SETUP (call from SetupPDOPosition, before ecrt_master_activate):
 *   create_mt_sdo_requests(sc) — registers both request objects
 *
 * TRIGGER (call from any goroutine, including during Op mode):
 *   trigger_mt_reset_step(step, value) — arms one request with a value
 *   get_mt_request_state(step)         — polls state (0=unused/busy, 1=success, -1=error)
 * ============================================================ */
 ...
 *
 * Returns:
 *   0       — request armed successfully
 *  -EINVAL  — request object not created (create_mt_sdo_requests not called)
 *  -EBUSY   — slave mailbox still processing prior request; caller should retry
 *
...
int trigger_mt_request_step(int step, uint32_t value) {
    ...
    if (state == EC_REQUEST_BUSY) {
        /* Slave mailbox not yet free — caller should sleep 1-2ms and retry */
        return -EBUSY;
    }
...
/* ============================================================
 * PDO offset setup functions — Option A returns CACHED IgH offsets.
...
int setup_pos_pdo(ec_domain_t *domain, uint16_t alias, uint16_t position,
    ...
    (void)domain; (void)alias; (void)position; (void)vendor_id; (void)product_code;
    if (!g_off_valid) return -EINVAL;
    *off_pos = g_off_pa;
    return 0;
}
